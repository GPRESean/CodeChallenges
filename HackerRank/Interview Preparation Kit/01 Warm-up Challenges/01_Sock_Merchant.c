...
int sockMerchant(int n, int ar_count, int* ar) {
    ...
    int ret;
    int *ar_loc = (int *)malloc(sizeof(int) * ar_count);
    memcpy(ar_loc, ar, sizeof(int) * ar_count);
    
    ret = 0;
    for(int i = 0; i < ar_count-1; i++)
    {
        // If the value is cleared, then skip it
        if(!ar_loc[i])
            continue;
        ...
        {
            // Found pair
            if(ar_loc[i] == ar_loc[j])
            {
                // Clear values of pair
                ar_loc[i] = ar_loc[j] = CLEAR_VAL;
                ret++;
                break;
            }
        }
